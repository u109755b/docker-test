...
# start
def start(env_directory, start_db=True, start_proxy=True):
    os.chdir(env_directory)
    with open('docker-compose.yml', 'r') as f:
        services = yaml.safe_load(f)["services"]

    peer_names = []
    for peer_name in services.keys():
        if start_db and peer_name.endswith("db"):
            peer_names.append(peer_name)
        if start_proxy and peer_name.endswith("proxy"):
            peer_names.append(peer_name)
...
def start_command(args):
    remove()
    update()
    start(args.env_directory, start_db=True, start_proxy=True)

def startdb_command(args):
    remove(remove_db=True, remove_proxy=False)
    start(args.env_directory, start_db=True, start_proxy=False)

def startproxy_command(args):
    remove(remove_db=False, remove_proxy=True)
    update()
    start(args.env_directory, start_db=False, start_proxy=True)
...
# command examples
# python3 commands.py start -e env_generator/TPCC_N3/
# python3 commands.py startdb -e env_generator/TPCC_N3/
# python3 commands.py startproxy -e env_generator/TPCC_N3/
