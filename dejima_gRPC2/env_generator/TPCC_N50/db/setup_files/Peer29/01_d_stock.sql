...
CREATE OR REPLACE FUNCTION public.stock_propagates_to_d_stock(txid int)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
  BEGIN
    DELETE FROM public.__dummy__d_stock_detected_deletions t where t.txid = $1;
    DELETE FROM public.__dummy__d_stock_detected_insertions t where t.txid = $1;
    PERFORM public.stock_propagate_updates_to_d_stock();
    RETURN true;
  END;
$$;

CREATE OR REPLACE FUNCTION public.d_stock_propagate(txid int)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
  BEGIN
    DELETE FROM public.__dummy__d_stock_detected_deletions t where t.txid = $1;
    DELETE FROM public.__dummy__d_stock_detected_insertions t where t.txid = $1;
    PERFORM public.d_stock_propagate_updates();
    RETURN true;
  END;
$$;

CREATE OR REPLACE FUNCTION public.remove_dummy_d_stock(txid int)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
