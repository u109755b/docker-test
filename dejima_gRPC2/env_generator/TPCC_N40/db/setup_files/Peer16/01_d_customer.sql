...
CREATE OR REPLACE FUNCTION public.customer_propagates_to_d_customer(txid int)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
  BEGIN
    DELETE FROM public.__dummy__d_customer_detected_deletions t where t.txid = $1;
    DELETE FROM public.__dummy__d_customer_detected_insertions t where t.txid = $1;
    PERFORM public.customer_propagate_updates_to_d_customer();
    RETURN true;
  END;
$$;

CREATE OR REPLACE FUNCTION public.d_customer_propagate(txid int)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
  BEGIN
    DELETE FROM public.__dummy__d_customer_detected_deletions t where t.txid = $1;
    DELETE FROM public.__dummy__d_customer_detected_insertions t where t.txid = $1;
    PERFORM public.d_customer_propagate_updates();
    RETURN true;
  END;
$$;

CREATE OR REPLACE FUNCTION public.remove_dummy_d_customer(txid int)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
