...
def countup_request(lineages, request_type, parent_peer):
    ...
    for lineage in set(lineages):
        ...
        request_count[lineage].append((request_type, parent_peer))
        if 3 < len(request_count[lineage]):
            request_count[lineage].popleft()
            # expansion test
            if request_type == "read" and parent_peer not in get_r_direction(lineage):
                ...
                if read_count > update_count: expansion_lineages.append(lineage)
                print(f"read {read_count} {update_count} {request_count[lineage]}")
            # contraction test
            if request_type == "update" and parent_peer in get_r_direction(lineage):
                ...
                for req in request_count[lineage]:
                    if req[0] == "update" and req[1] == parent_peer: update_count += 1
                    if req[0] == "read" and req[1] != parent_peer: read_count += 1
                if read_count < update_count: contraction_lineages.append(lineage)
                print(f"update {read_count} {update_count} {request_count[lineage]}")
    # return []
