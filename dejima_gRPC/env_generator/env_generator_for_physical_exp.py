...
# main
output_dir_path = '{}_N{}_B{}_Straight'.format(benchmark, N, B)
...
for j in range(1,cond_n+1):
    with open("src/{}/sql/dnum_cond{}.sql".format(benchmark,j)) as f:
...
for i in range(1,N+1):
    ...
    if not i == 1:
        ...
        with open("{}/db/setup_files/Peer{}/01_{}.sql".format(output_dir_path, target_node, dt_name), mode="w") as f:
            f.write(birds_sqls[cond].replace("dnum", dt_name).replace("<border>", B))
