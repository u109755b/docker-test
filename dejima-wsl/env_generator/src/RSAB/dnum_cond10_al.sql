...
FROM (SELECT dnum_a16_0.COL0 AS COL0, dnum_a16_0.COL1 AS COL1, dnum_a16_0.COL2 AS COL2, dnum_a16_0.COL3 AS COL3, dnum_a16_0.COL4 AS COL4, dnum_a16_0.COL5 AS COL5, dnum_a16_0.COL6 AS COL6, dnum_a16_0.COL7 AS COL7, dnum_a16_0.COL8 AS COL8, dnum_a16_0.COL9 AS COL9, dnum_a16_0.COL10 AS COL10, dnum_a16_0.COL11 AS COL11, dnum_a16_0.COL12 AS COL12, dnum_a16_0.COL13 AS COL13, dnum_a16_0.COL14 AS COL14, dnum_a16_0.COL15 AS COL15 
...
WHERE mt_a16_0.COND10  <  <border> ) AS dnum_a16_0  ) AS __dummy__;
...
CREATE OR REPLACE FUNCTION public.mt_materialization_for_dnum()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
text_var3 text;
BEGIN
    IF NOT EXISTS (SELECT * FROM information_schema.tables WHERE table_name = '__tmp_delta_ins_mt_for_dnum' OR table_name = '__tmp_delta_del_mt_for_dnum')
    THEN
        -- RAISE LOG 'execute procedure mt_materialization_for_dnum';
        CREATE TEMPORARY TABLE IF NOT EXISTS __tmp_delta_ins_mt_for_dnum ( LIKE public.mt ) WITH OIDS ON COMMIT DELETE ROWS;
        CREATE TEMPORARY TABLE IF NOT EXISTS __tmp_delta_del_mt_for_dnum ( LIKE public.mt ) WITH OIDS ON COMMIT DELETE ROWS;
...
DROP TRIGGER IF EXISTS mt_trigger_materialization_for_dnum ON public.mt;
CREATE TRIGGER mt_trigger_materialization_for_dnum
    BEFORE INSERT OR UPDATE OR DELETE ON
    public.mt FOR EACH STATEMENT EXECUTE PROCEDURE public.mt_materialization_for_dnum();

CREATE OR REPLACE FUNCTION public.mt_update_for_dnum()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
text_var3 text;
BEGIN
    -- RAISE LOG 'execute procedure mt_update_for_dnum';
    IF NOT EXISTS (SELECT * FROM information_schema.tables WHERE table_name = 'dnum_delta_action_flag') THEN
        ...
        END IF;
        DELETE FROM __tmp_delta_del_mt_for_dnum WHERE ROW(V,L,D,R,P,LINEAGE,COND1,COND2,COND3,COND4,COND5,COND6,COND7,COND8,COND9,COND10) = NEW;
        INSERT INTO __tmp_delta_ins_mt_for_dnum SELECT (NEW).*; 
        ELSIF TG_OP = 'UPDATE' THEN
        IF (SELECT count(*) FILTER (WHERE j.value = jsonb 'null') FROM  jsonb_each(to_jsonb(NEW)) j) > 0 THEN 
            RAISE check_violation USING MESSAGE = 'Invalid update: null value is not accepted';
        END IF;
        DELETE FROM __tmp_delta_ins_mt_for_dnum WHERE ROW(V,L,D,R,P,LINEAGE,COND1,COND2,COND3,COND4,COND5,COND6,COND7,COND8,COND9,COND10) = OLD;
        INSERT INTO __tmp_delta_del_mt_for_dnum SELECT (OLD).*;
        DELETE FROM __tmp_delta_del_mt_for_dnum WHERE ROW(V,L,D,R,P,LINEAGE,COND1,COND2,COND3,COND4,COND5,COND6,COND7,COND8,COND9,COND10) = NEW;
        INSERT INTO __tmp_delta_ins_mt_for_dnum SELECT (NEW).*; 
...
DROP TRIGGER IF EXISTS mt_trigger_update_for_dnum ON public.mt;
CREATE TRIGGER mt_trigger_update_for_dnum
    AFTER INSERT OR UPDATE OR DELETE ON
    public.mt FOR EACH ROW EXECUTE PROCEDURE public.mt_update_for_dnum();

CREATE OR REPLACE FUNCTION public.mt_detect_update_on_dnum()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
delta_del_size int;
BEGIN
IF NOT EXISTS (SELECT * FROM information_schema.tables WHERE table_name = 'mt_detect_update_on_dnum_flag') THEN
    CREATE TEMPORARY TABLE mt_detect_update_on_dnum_flag ON COMMIT DROP AS (SELECT true as finish);
    IF NOT EXISTS (SELECT * FROM information_schema.tables WHERE table_name = 'dnum_delta_action_flag') THEN
        SELECT array_agg(tbl) INTO array_delta_ins FROM __tmp_delta_ins_mt_for_dnum AS tbl;
        select count(*) INTO delta_ins_size FROM __tmp_delta_ins_mt_for_dnum;
        SELECT array_agg(tbl) INTO array_delta_del FROM __tmp_delta_del_mt_for_dnum tbl;
        select count(*) INTO delta_del_size FROM __tmp_delta_del_mt_for_dnum;

        WITH __tmp_delta_ins_mt_for_dnum_ar as (SELECT * FROM unnest(array_delta_ins) as array_delta_ins_alias limit delta_ins_size), 
        __tmp_delta_del_mt_for_dnum_ar as (SELECT * FROM unnest(array_delta_del) as array_delta_del_alias limit delta_del_size)
...
FROM (SELECT part_ins_dnum_a16_0.COL0 AS COL0, part_ins_dnum_a16_0.COL1 AS COL1, part_ins_dnum_a16_0.COL2 AS COL2, part_ins_dnum_a16_0.COL3 AS COL3, part_ins_dnum_a16_0.COL4 AS COL4, part_ins_dnum_a16_0.COL5 AS COL5, part_ins_dnum_a16_0.COL6 AS COL6, part_ins_dnum_a16_0.COL7 AS COL7, part_ins_dnum_a16_0.COL8 AS COL8, part_ins_dnum_a16_0.COL9 AS COL9, part_ins_dnum_a16_0.COL10 AS COL10, part_ins_dnum_a16_0.COL11 AS COL11, part_ins_dnum_a16_0.COL12 AS COL12, part_ins_dnum_a16_0.COL13 AS COL13, part_ins_dnum_a16_0.COL14 AS COL14, part_ins_dnum_a16_0.COL15 AS COL15 
...
FROM (SELECT __tmp_delta_ins_mt_for_dnum_ar_a16_0.V AS COL0, __tmp_delta_ins_mt_for_dnum_ar_a16_0.L AS COL1, __tmp_delta_ins_mt_for_dnum_ar_a16_0.D AS COL2, __tmp_delta_ins_mt_for_dnum_ar_a16_0.R AS COL3, __tmp_delta_ins_mt_for_dnum_ar_a16_0.P AS COL4, __tmp_delta_ins_mt_for_dnum_ar_a16_0.LINEAGE AS COL5, __tmp_delta_ins_mt_for_dnum_ar_a16_0.COND1 AS COL6, __tmp_delta_ins_mt_for_dnum_ar_a16_0.COND2 AS COL7, __tmp_delta_ins_mt_for_dnum_ar_a16_0.COND3 AS COL8, __tmp_delta_ins_mt_for_dnum_ar_a16_0.COND4 AS COL9, __tmp_delta_ins_mt_for_dnum_ar_a16_0.COND5 AS COL10, __tmp_delta_ins_mt_for_dnum_ar_a16_0.COND6 AS COL11, __tmp_delta_ins_mt_for_dnum_ar_a16_0.COND7 AS COL12, __tmp_delta_ins_mt_for_dnum_ar_a16_0.COND8 AS COL13, __tmp_delta_ins_mt_for_dnum_ar_a16_0.COND9 AS COL14, __tmp_delta_ins_mt_for_dnum_ar_a16_0.COND10 AS COL15 
FROM __tmp_delta_ins_mt_for_dnum_ar AS __tmp_delta_ins_mt_for_dnum_ar_a16_0 
WHERE __tmp_delta_ins_mt_for_dnum_ar_a16_0.COND10  <  <border> ) AS p_0_a16_0  ) AS part_ins_dnum_a16_0  ) AS __dummy__) AS tbl;
...
FROM (SELECT part_del_dnum_a16_0.COL0 AS COL0, part_del_dnum_a16_0.COL1 AS COL1, part_del_dnum_a16_0.COL2 AS COL2, part_del_dnum_a16_0.COL3 AS COL3, part_del_dnum_a16_0.COL4 AS COL4, part_del_dnum_a16_0.COL5 AS COL5, part_del_dnum_a16_0.COL6 AS COL6, part_del_dnum_a16_0.COL7 AS COL7, part_del_dnum_a16_0.COL8 AS COL8, part_del_dnum_a16_0.COL9 AS COL9, part_del_dnum_a16_0.COL10 AS COL10, part_del_dnum_a16_0.COL11 AS COL11, part_del_dnum_a16_0.COL12 AS COL12, part_del_dnum_a16_0.COL13 AS COL13, part_del_dnum_a16_0.COL14 AS COL14, part_del_dnum_a16_0.COL15 AS COL15 
FROM (SELECT p_0_a16_0.COL0 AS COL0, p_0_a16_0.COL1 AS COL1, p_0_a16_0.COL2 AS COL2, p_0_a16_0.COL3 AS COL3, p_0_a16_0.COL4 AS COL4, p_0_a16_0.COL5 AS COL5, p_0_a16_0.COL6 AS COL6, p_0_a16_0.COL7 AS COL7, p_0_a16_0.COL8 AS COL8, p_0_a16_0.COL9 AS COL9, p_0_a16_0.COL10 AS COL10, p_0_a16_0.COL11 AS COL11, p_0_a16_0.COL12 AS COL12, p_0_a16_0.COL13 AS COL13, p_0_a16_0.COL14 AS COL14, p_0_a16_0.COL15 AS COL15 
FROM (SELECT __tmp_delta_del_mt_for_dnum_ar_a16_0.V AS COL0, __tmp_delta_del_mt_for_dnum_ar_a16_0.L AS COL1, __tmp_delta_del_mt_for_dnum_ar_a16_0.D AS COL2, __tmp_delta_del_mt_for_dnum_ar_a16_0.R AS COL3, __tmp_delta_del_mt_for_dnum_ar_a16_0.P AS COL4, __tmp_delta_del_mt_for_dnum_ar_a16_0.LINEAGE AS COL5, __tmp_delta_del_mt_for_dnum_ar_a16_0.COND1 AS COL6, __tmp_delta_del_mt_for_dnum_ar_a16_0.COND2 AS COL7, __tmp_delta_del_mt_for_dnum_ar_a16_0.COND3 AS COL8, __tmp_delta_del_mt_for_dnum_ar_a16_0.COND4 AS COL9, __tmp_delta_del_mt_for_dnum_ar_a16_0.COND5 AS COL10, __tmp_delta_del_mt_for_dnum_ar_a16_0.COND6 AS COL11, __tmp_delta_del_mt_for_dnum_ar_a16_0.COND7 AS COL12, __tmp_delta_del_mt_for_dnum_ar_a16_0.COND8 AS COL13, __tmp_delta_del_mt_for_dnum_ar_a16_0.COND9 AS COL14, __tmp_delta_del_mt_for_dnum_ar_a16_0.COND10 AS COL15 
FROM __tmp_delta_del_mt_for_dnum_ar AS __tmp_delta_del_mt_for_dnum_ar_a16_0 
WHERE __tmp_delta_del_mt_for_dnum_ar_a16_0.COND10  <  <border> ) AS p_0_a16_0  ) AS part_del_dnum_a16_0  ) AS __dummy__) AS tbl;
        ...
        IF (insertion_data IS DISTINCT FROM '[]') OR (deletion_data IS DISTINCT FROM '[]') THEN 
            ...
            IF NOT (user_name = 'dejima') THEN 
                ...
                IF result = 'true' THEN 
                    DROP TABLE __tmp_delta_ins_mt_for_dnum;
                    DROP TABLE __tmp_delta_del_mt_for_dnum;
                ELSE
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS text_var1 = RETURNED_SQLSTATE,
                                text_var2 = PG_EXCEPTION_DETAIL,
                                text_var3 = MESSAGE_TEXT;
        RAISE SQLSTATE 'DA000' USING MESSAGE = 'error on the function public.mt_detect_update_on_dnum() ; error code: ' || text_var1 || ' ; ' || text_var2 ||' ; ' || text_var3;
        RETURN NULL;
END;
$$;

DROP TRIGGER IF EXISTS mt_detect_update_on_dnum ON public.mt;
CREATE CONSTRAINT TRIGGER mt_detect_update_on_dnum
    AFTER INSERT OR UPDATE OR DELETE ON
    public.mt DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE PROCEDURE public.mt_detect_update_on_dnum();

CREATE OR REPLACE FUNCTION public.mt_propagate_updates_to_dnum ()
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
  BEGIN
    SET CONSTRAINTS public.mt_detect_update_on_dnum IMMEDIATE;
    SET CONSTRAINTS public.mt_detect_update_on_dnum DEFERRED;
    DROP TABLE IF EXISTS mt_detect_update_on_dnum_flag;
    RETURN true;
  END;
$$;
...
FROM __tmp_delta_del_dnum_ar AS __tmp_delta_del_dnum_ar_a16_0, public.mt AS mt_a16_1 
WHERE mt_a16_1.V = __tmp_delta_del_dnum_ar_a16_0.V AND mt_a16_1.L = __tmp_delta_del_dnum_ar_a16_0.L AND mt_a16_1.D = __tmp_delta_del_dnum_ar_a16_0.D AND mt_a16_1.R = __tmp_delta_del_dnum_ar_a16_0.R AND mt_a16_1.P = __tmp_delta_del_dnum_ar_a16_0.P AND mt_a16_1.LINEAGE = __tmp_delta_del_dnum_ar_a16_0.LINEAGE AND mt_a16_1.COND1 = __tmp_delta_del_dnum_ar_a16_0.COND1 AND mt_a16_1.COND2 = __tmp_delta_del_dnum_ar_a16_0.COND2 AND mt_a16_1.COND3 = __tmp_delta_del_dnum_ar_a16_0.COND3 AND mt_a16_1.COND4 = __tmp_delta_del_dnum_ar_a16_0.COND4 AND mt_a16_1.COND5 = __tmp_delta_del_dnum_ar_a16_0.COND5 AND mt_a16_1.COND6 = __tmp_delta_del_dnum_ar_a16_0.COND6 AND mt_a16_1.COND7 = __tmp_delta_del_dnum_ar_a16_0.COND7 AND mt_a16_1.COND8 = __tmp_delta_del_dnum_ar_a16_0.COND8 AND mt_a16_1.COND9 = __tmp_delta_del_dnum_ar_a16_0.COND9 AND mt_a16_1.COND10 = __tmp_delta_del_dnum_ar_a16_0.COND10 AND mt_a16_1.COND10  <  <border> ) AS p_0_a16_0  ) AS delta_del_mt_a16_0  ) AS delta_del_mt_extra_alias) AS tbl;
...
FROM (SELECT p_0_a16_0.COL0 AS COL0, p_0_a16_0.COL1 AS COL1, p_0_a16_0.COL2 AS COL2, p_0_a16_0.COL3 AS COL3, p_0_a16_0.COL4 AS COL4, p_0_a16_0.COL5 AS COL5, p_0_a16_0.COL6 AS COL6, p_0_a16_0.COL7 AS COL7, p_0_a16_0.COL8 AS COL8, p_0_a16_0.COL9 AS COL9, p_0_a16_0.COL10 AS COL10, p_0_a16_0.COL11 AS COL11, p_0_a16_0.COL12 AS COL12, p_0_a16_0.COL13 AS COL13, p_0_a16_0.COL14 AS COL14, p_0_a16_0.COL15 AS COL15 
FROM (SELECT __tmp_delta_ins_dnum_ar_a16_0.V AS COL0, __tmp_delta_ins_dnum_ar_a16_0.L AS COL1, __tmp_delta_ins_dnum_ar_a16_0.D AS COL2, __tmp_delta_ins_dnum_ar_a16_0.R AS COL3, __tmp_delta_ins_dnum_ar_a16_0.P AS COL4, __tmp_delta_ins_dnum_ar_a16_0.LINEAGE AS COL5, __tmp_delta_ins_dnum_ar_a16_0.COND1 AS COL6, __tmp_delta_ins_dnum_ar_a16_0.COND2 AS COL7, __tmp_delta_ins_dnum_ar_a16_0.COND3 AS COL8, __tmp_delta_ins_dnum_ar_a16_0.COND4 AS COL9, __tmp_delta_ins_dnum_ar_a16_0.COND5 AS COL10, __tmp_delta_ins_dnum_ar_a16_0.COND6 AS COL11, __tmp_delta_ins_dnum_ar_a16_0.COND7 AS COL12, __tmp_delta_ins_dnum_ar_a16_0.COND8 AS COL13, __tmp_delta_ins_dnum_ar_a16_0.COND9 AS COL14, __tmp_delta_ins_dnum_ar_a16_0.COND10 AS COL15 
FROM __tmp_delta_ins_dnum_ar AS __tmp_delta_ins_dnum_ar_a16_0 
WHERE __tmp_delta_ins_dnum_ar_a16_0.COND10  <  <border> AND NOT EXISTS ( SELECT * 
FROM public.mt AS mt_a16 
WHERE mt_a16.V = __tmp_delta_ins_dnum_ar_a16_0.V AND mt_a16.L = __tmp_delta_ins_dnum_ar_a16_0.L AND mt_a16.D = __tmp_delta_ins_dnum_ar_a16_0.D AND mt_a16.R = __tmp_delta_ins_dnum_ar_a16_0.R AND mt_a16.P = __tmp_delta_ins_dnum_ar_a16_0.P AND mt_a16.LINEAGE = __tmp_delta_ins_dnum_ar_a16_0.LINEAGE AND mt_a16.COND1 = __tmp_delta_ins_dnum_ar_a16_0.COND1 AND mt_a16.COND2 = __tmp_delta_ins_dnum_ar_a16_0.COND2 AND mt_a16.COND3 = __tmp_delta_ins_dnum_ar_a16_0.COND3 AND mt_a16.COND4 = __tmp_delta_ins_dnum_ar_a16_0.COND4 AND mt_a16.COND5 = __tmp_delta_ins_dnum_ar_a16_0.COND5 AND mt_a16.COND6 = __tmp_delta_ins_dnum_ar_a16_0.COND6 AND mt_a16.COND7 = __tmp_delta_ins_dnum_ar_a16_0.COND7 AND mt_a16.COND8 = __tmp_delta_ins_dnum_ar_a16_0.COND8 AND mt_a16.COND9 = __tmp_delta_ins_dnum_ar_a16_0.COND9 AND mt_a16.COND10 = __tmp_delta_ins_dnum_ar_a16_0.COND10 ) ) AS p_0_a16_0  ) AS delta_ins_mt_a16_0  ) AS delta_ins_mt_extra_alias) AS tbl; 
...
AS $$
  ...
  BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
      ...
      END IF;
      DELETE FROM __tmp_delta_del_dnum WHERE ROW(V,L,D,R,P,LINEAGE,COND1,COND2,COND3,COND4,COND5,COND6,COND7,COND8,COND9,COND10) = NEW;
      INSERT INTO __tmp_delta_ins_dnum SELECT (NEW).*; 
    ELSIF TG_OP = 'UPDATE' THEN
      IF (SELECT count(*) FILTER (WHERE j.value = jsonb 'null') FROM  jsonb_each(to_jsonb(NEW)) j) > 0 THEN 
        RAISE check_violation USING MESSAGE = 'Invalid update on view: view does not accept null value';
      END IF;
      DELETE FROM __tmp_delta_ins_dnum WHERE ROW(V,L,D,R,P,LINEAGE,COND1,COND2,COND3,COND4,COND5,COND6,COND7,COND8,COND9,COND10) = OLD;
...
CREATE CONSTRAINT TRIGGER __zzz_clean_dnum
    AFTER INSERT OR UPDATE OR DELETE ON
    public.mt DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE PROCEDURE public.clean_dummy_dnum();
