...
FROM (SELECT v_a2_0.COL0 AS COL0, v_a2_0.COL1 AS COL1 
FROM (SELECT p_0_a2_0.COL0 AS COL0, p_0_a2_0.COL1 AS COL1 
FROM (SELECT s1_a2_0.X AS COL0, s1_a2_0.Y AS COL1 
FROM public.s1 AS s1_a2_0   UNION SELECT s2_a2_0.X AS COL0, s2_a2_0.Y AS COL1 
FROM public.s2 AS s2_a2_0  ) AS p_0_a2_0  ) AS v_a2_0  ) AS __dummy__;
  ...
  BEGIN
    IF NOT EXISTS (SELECT * FROM information_schema.tables WHERE table_name = 'v_delta_action_flag') THEN
        ...
        CREATE TEMPORARY TABLE Δ_del_s1 WITH OIDS ON COMMIT DROP AS SELECT (ROW(COL0,COL1) :: public.s1).* 
            FROM (SELECT Δ_del_s1_a2_0.COL0 AS COL0, Δ_del_s1_a2_0.COL1 AS COL1 
FROM (SELECT s1_a2_0.X AS COL0, s1_a2_0.Y AS COL1 
FROM public.s1 AS s1_a2_0 
WHERE NOT EXISTS ( SELECT * 
FROM (SELECT v_a2_0.X AS COL0, v_a2_0.Y AS COL1 
FROM public.v AS v_a2_0 
WHERE NOT EXISTS ( SELECT * 
FROM __temp__Δ_del_v AS __temp__Δ_del_v_a2 
WHERE __temp__Δ_del_v_a2.Y = v_a2_0.Y AND __temp__Δ_del_v_a2.X = v_a2_0.X )  UNION SELECT __temp__Δ_ins_v_a2_0.X AS COL0, __temp__Δ_ins_v_a2_0.Y AS COL1 
...
CREATE TEMPORARY TABLE Δ_del_s2 WITH OIDS ON COMMIT DROP AS SELECT (ROW(COL0,COL1) :: public.s2).* 
            FROM (SELECT Δ_del_s2_a2_0.COL0 AS COL0, Δ_del_s2_a2_0.COL1 AS COL1 
FROM (SELECT s2_a2_0.X AS COL0, s2_a2_0.Y AS COL1 
...
CREATE TEMPORARY TABLE Δ_ins_s1 WITH OIDS ON COMMIT DROP AS SELECT (ROW(COL0,COL1) :: public.s1).* 
            FROM (SELECT Δ_ins_s1_a2_0.COL0 AS COL0, Δ_ins_s1_a2_0.COL1 AS COL1 
FROM (SELECT new_v_a2_0.COL0 AS COL0, new_v_a2_0.COL1 AS COL1 
FROM (SELECT v_a2_0.X AS COL0, v_a2_0.Y AS COL1 
FROM public.v AS v_a2_0 
WHERE NOT EXISTS ( SELECT * 
FROM __temp__Δ_del_v AS __temp__Δ_del_v_a2 
